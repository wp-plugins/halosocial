...
class HALOAssetHelper
{
	...
	static $queuedCss = array();
	static $loadedJs = array();
	static $queuedJs = array();
	...
	*/
	public static function loadDefaultCss($layout = 'default'){
		...
		//halo css
		self::addCss('assets/css/override.css');
	...
	*/
	public static function enqueueStyle(){
		//bundle css files
		if(!HALOConfig::isDev()){
			$files = array_diff_assoc(self::$loadedCss, self::$queuedCss);
			if(empty($files)) return;
			$bundle = self::bundleAssets('assets/css/', $files, 'css');
			$path = $bundle[1];
			if(!$path) {
				//update the queueded list
				self::$queuedCss = self::$loadedCss;
				wp_enqueue_style( $bundle[0], File::toUrl($path) );
			} else {
				//fallback option
				foreach($files as $hash => $file) {
					wp_enqueue_style( $hash, File::toUrl($file) );
				}
			}
		}	
	}
	...
	*/
	public static function enqueueJs(){
		//bundle js files
		if(!HALOConfig::isDev()){
			$files = array_diff_assoc(self::$loadedJs, self::$queuedJs);
			if(empty($files)) return;
			$bundle = self::bundleAssets('assets/js/bundle_/', $files, 'js');
			$path = $bundle[1];
			$in_footer =  HALOConfig::get('global.footerScript', true);
			if($path) {
				//update the queueded list
				self::$queuedJs = self::$loadedJs;
				wp_enqueue_script( $bundle[0], File::toUrl($path), array( 'jquery' ), false, $in_footer );
			} else {
				//fallback option
				foreach($files as $hash => $file) {
					wp_enqueue_script( $hash, File::toUrl($file), array( 'jquery' ), false, $in_footer );
				}
			}
		}		
	}
	...
	*/
	...
	/*
		print inline javascript variable required for js init
	*/
	public static function printInlineJSInit(){
		echo "<script>
				var halo_feature_message = 1;
				var halo_popup_message_win = 1;
				var halo_my_id = " . get_current_user_id() . "
				jQuery(document).ready(function(){
					halo.message.init(document);
				});
			</script>";
	}
}
