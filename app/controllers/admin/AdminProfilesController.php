...
class AdminProfilesController extends AdminController
{
    ...
    public function getIndex()
    {
		...
		if(HALOAuth::can('feature.profile')){
			HALOToolbar::addToolbar('Add Profile','halo-btn-success','','halo.profile.showEditProfileForm(0)','plus');
			HALOToolbar::addToolbar('Delete Profile','halo-btn-danger','',"halo.popup.confirmDelete('Delete Profile Confirm','Are you sure to delete this profile','halo.profile.deleteSelectedProfile()')",'times');
		} else {
			HALOUtilHelper::getVersionToolbar();
		}

        // Get activated plugins
        $parsedNames = HALOProfileModel::getAvailableTypes();
	...
	public function getFields($profileId){
		...
		// Toolbar
		if(HALOAuth::can('feature.profile')){
			HALOToolbar::addToolbar('Back','halo-btn-info',Url::to('?app=admin&view=profiles'),'','undo');
			HALOToolbar::addToolbar('Attach Field','halo-btn-primary','','halo.profile.showAttachFieldForm('.$profileId.',0)','plus');
			HALOToolbar::addToolbar('Detach Field','halo-btn-danger','',"halo.popup.confirmDelete('Detach Field Confirm','Are you sure to detach these fields','halo.profile.detachSelectedField(".$profileId.")')",'times');
			HALOToolbar::addToolbar('Add New Field','halo-btn-success','','halo.profile.showAddNewFieldForm('.$profileId.',0)','plus');
		} else {
			HALOUtilHelper::getVersionToolbar();
		}
