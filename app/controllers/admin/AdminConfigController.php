...
class AdminConfigController extends AdminController
{
	...
	public function getIndex()
	{
		...
		HALOToolbar::addToolbar('Save','halo-btn-primary','',"halo.form.submit('halo-admin-form')",'floppy-o');
		HALOToolbar::addToolbar('Cancel','halo-btn-default','',"halo.util.reload()",'undo');
		HALOToolbar::addToolbar('Clear Cache','halo-btn-danger','',"halo.tools.clearCache()",'chain-broken');
	...
	*/
	protected function getGlobalCfg(){
		...
		$section = HALOUIBuilder::getInstance('activities','config.section',array('title'=>__halotext('Activities')))
							->addUI('global.activityQueryLimit', HALOUIBuilder::getInstance('','form.text',array('name'=>'global.activityQueryLimit',
																							'title'=>__halotext('Activity Display Limit'),
																							'helptext'=>__halotext('Limit the number of activities returned per request'),
																							'value'=>HALOConfig::get('global.activityQueryLimit', 12))))
	...
	protected function getThemeCfg(){
		$themeCfg = new stdClass();
		$themeCfg->title = "Themes";
		$themeCfg->icon = "";
		$themeCfg->builder = HALOUIBuilder::getInstance('theme_cfg','config.group',array('name'=>'theme_cfg'));

		$themeActions = HALOUIBuilder::getInstance('theme_actions', 'content', array('class' => 'block-center halo-theme-actions', 'zone' => 'halo-theme-actions'));
		$themeActions->addUI('theme_reset',HALOUIBuilder::getInstance('', 'form.button', array('title' => __halotext('Reset'), 
																									'class' => 'halo-btn-danger halo-theme-reset-btn',
																									'inline' => 1,
																									'onClick' => 'halo.theme.resetCustomize()')));
		$themeActions->addUI('theme_cancel',HALOUIBuilder::getInstance('', 'form.button', array('title' => __halotext('Cancel'), 
																									'class' => 'halo-theme-cancel-btn',
																									'inline' => 1,
																									'onClick' => 'halo.util.reload()')));
		$themeActions->addUI('theme_apply',HALOUIBuilder::getInstance('', 'form.button', array('title' => __halotext('Apply'), 
																									'class' => 'halo-btn-primary halo-theme-apply-btn',
																									'inline' => 1, 
																									'onClick' => 'halo.theme.customize()')));
		$themeActions->addUI('theme_separator',HALOUIBuilder::getInstance('', 'clearfix'));
		$themeCfg->builder->addUI('theme_actions', $themeActions);
		
		//main colors section
		$section = HALOUIBuilder::getInstance('','config.section',array('title'=>__halotext('Color Schema')));
		
		$section->addUI('theme.color.default', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.default',
																				'title'=>__halotext('Default Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Default Color'),
																				'value'=>HALOConfig::get('theme.color.default', strtoupper('#F5F5F5')))));
		$section->addUI('theme.color.primary', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.primary',
																				'title'=>__halotext('Primary Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Primary Color'),
																				'value'=>HALOConfig::get('theme.color.primary', strtoupper('#428bca')))));
		$section->addUI('theme.color.success', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.success',
																				'title'=>__halotext('Success Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Success Color'),
																				'value'=>HALOConfig::get('theme.color.success', strtoupper('#5cb85c')))));
		$section->addUI('theme.color.info', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.info',
																				'title'=>__halotext('Info Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Info Color'),
																				'value'=>HALOConfig::get('theme.color.info', strtoupper('#5bc0de')))));
		$section->addUI('theme.color.warning', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.warning',
																				'title'=>__halotext('Warning Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Warning Color'),
																				'value'=>HALOConfig::get('theme.color.warning', strtoupper('#f0ad4e')))));
		$section->addUI('theme.color.danger', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.danger',
																				'title'=>__halotext('Danger Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Danger Color'),
																				'value'=>HALOConfig::get('theme.color.danger', strtoupper('#d9534f')))));
		$themeCfg->builder->addUI('section@array',$section);
						
		//font section
		$section = HALOUIBuilder::getInstance('','config.section',array('title'=>__halotext('Typography')));
		
		$section->addUI('theme.color.font', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.font',
																				'title'=>__halotext('Font Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Font Color'),
																				'value'=>HALOConfig::get('theme.color.font', strtoupper('#565a5c')))));
		$section->addUI('theme.color.background', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.background',
																				'title'=>__halotext('Background Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Background Color'),
																				'value'=>HALOConfig::get('theme.color.background', strtoupper('#ffffff')))));
		$section->addUI('theme.color.border', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.color.border',
																				'title'=>__halotext('Border Color'),
																				'class'=> 'halo-color-input',
																				'helptext'=>__halotext('Border Color'),
																				'value'=>HALOConfig::get('theme.color.border', strtoupper('#dce0e0')))));
		$section->addUI('theme.font.size', HALOUIBuilder::getInstance('','form.number',array('name'=>'theme.font.size',
																				'title'=>__halotext('Font Size'),
																				'class'=> '',
																				'helptext'=>__halotext('Font Size'),
																				'value'=>HALOConfig::get('theme.font.size', 14))));

		$themeCfg->builder->addUI('section@array',$section);
		
		//theme override
		$section = HALOUIBuilder::getInstance('','config.section',array('title'=>__halotext('Theme Override')));
		
		$section->addUI('theme.css.override', HALOUIBuilder::getInstance('','form.textarea',array('name'=>'theme.css.override',
																				'title'=>__halotext('CSS Override'),
																				'class'=> '',
																				'rows' => 10,
																				'helptext'=>__halotext('Put your customization css here'),
																				'value'=>HALOConfig::get('theme.css.override', ''))));

		$section->addUI('theme.image.banner', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.image.banner',
																				'title'=>__halotext('Banner Image'),
																				'class'=> '',
																				'helptext'=>__halotext('Change default banner image'),
																				'value'=>HALOConfig::get('theme.image.banner', ''))));

		$section->addUI('theme.image.user_avatar', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.image.user_avatar',
																				'title'=>__halotext('Default User Avatar'),
																				'class'=> '',
																				'helptext'=>__halotext('Change default user avatar image'),
																				'value'=>HALOConfig::get('theme.image.user_avatar', ''))));

		$section->addUI('theme.image.user_cover', HALOUIBuilder::getInstance('','form.text',array('name'=>'theme.image.user_cover',
																				'title'=>__halotext('Default User Cover'),
																				'class'=> '',
																				'helptext'=>__halotext('Change default user cover image'),
																				'value'=>HALOConfig::get('theme.image.user_cover', ''))));

		$themeCfg->builder->addUI('section@array',$section);
		
		Event::fire('system.loadThemeSettings', array(&$themeCfg));
						
		return $themeCfg;	
	}
	
	/*
		ajax handle to compile css files
	*/
	public function ajaxCustomizeTheme($data, $index = 0) {

		require_once HALO_PLUGIN_PATH . "app/library/halo/scss/scss.php";
		//track start time
		$startTime = time();
		...
		$themeSettings->sassArr = array();
		$themeSettings->varsMap = array();
		$reset = empty($data)?1:0;
		
		Event::fire('system.customizeTheme', array(&$data, &$themeSettings));
		
		$variables = array();
		foreach($themeSettings->varsMap as $var) {
			if(isset($var['cfgKey']) && isset($data[$var['cfgKey']]) && isset($var['var'])) {
				$variables[$var['var']] = $data[$var['cfgKey']];
			}
		}
		try {
			while(isset($themeSettings->sassArr[$index]) && ($startTime + 25) > time()){
				$path = isset($themeSettings->sassArr[$index]['path'])?$themeSettings->sassArr[$index]['path']:HALO_PLUGIN_PATH . '/app/views/default/assets/css/';
				$format = isset($themeSettings->sassArr[$index]['format'])?$themeSettings->sassArr[$index]['format']:'scss_formatter';
				HALOSassCompiler::setSassFolder($path);
				HALOSassCompiler::setVariables($variables);
				
				if(isset($themeSettings->sassArr[$index]['append']) && !empty($themeSettings->sassArr[$index]['append'])){
					//add additonal extra buffer is provided
					HALOSassCompiler::appendBuffer($themeSettings->sassArr[$index]['append']);				
				}
				//run compiler
				HALOSassCompiler::run($themeSettings->sassArr[$index]['file'], $themeSettings->sassArr[$index]['scss'], $themeSettings->sassArr[$index]['css'], $format);
				$index ++;
			}
		} catch(\Exception $e){
		...
		if($index < count($themeSettings->sassArr)){
			HALOResponse::addScriptCall('halo.theme.customize', $index, $reset);
		} else {
			//store settings
			if(!$reset) {
				foreach($themeSettings->varsMap as $var) {
					if(isset($var['cfgKey']) && isset($data[$var['cfgKey']])) {
						HALOConfig::set($var['cfgKey'], $data[$var['cfgKey']]);
						HALOConfig::store();
					}
				}
			}
			HALOResponse::addScriptCall('halo.theme.doneCustomize');
		}
	...
	/*
		ajax handle to compile css files
	*/
	public function ajaxResetTheme($data = array()) {
		...
		//store settings
		foreach($themeSettings->varsMap as $var) {
			HALOConfig::set($var['cfgKey'], null);
			HALOConfig::store();
		}
		
		return $this->ajaxCustomizeTheme(array(), 0);
