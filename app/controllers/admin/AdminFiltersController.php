...
class AdminFiltersController extends AdminController {
    ...
    public function getIndex()
    {
		...
		// Toolbar
		// HALOToolbar::addToolbar(__halotext('Reset Filters'),'halo-btn-success','','halo.filter.resetFilters()','refresh');
		// HALOToolbar::addToolbar(__halotext('Add Filter'),'','','halo.filter.showEditFilterForm(0)','plus');
		// HALOToolbar::addToolbar(__halotext('Delete Filter'),'halo-btn-danger','',"halo.popup.confirmDelete('Delete Filter Confirm','Are you sure to delete this filter?','halo.filter.deleteSelectedFilter()')",'times');
		
        // Grab all the filters
        $filters = new HALOFilterModel();
		$filters = $filters->where('name', 'not like', 'admin.%');
	...
	public function ajaxShowEditFilterForm($filterId=0){
		...
		$filterConfig = HALOFilter::getFilterConfiguration();
		$filterValue = array('display' => $filter->on_display_handler, 'apply' => $filter->on_apply_handler, 'name' => $filter->name);
		$filterParamsConfig = array_filter($filterConfig->filters, function($config) use($filterValue){
		...
		$filterParamsConfig = array_shift($filterParamsConfig);
		$filterName = isset($filterParamsConfig['title'])?$filterParamsConfig['title']:__halotext('Unknown');
		$filterParamsConfig = isset($filterParamsConfig['params'])?$filterParamsConfig['params']:array();
		
		$builder = HALOUIBuilder::getInstance('editFilter','form.form',array('name'=>'popupForm'))
					->addUI('filter', HALOUIBuilder::getInstance('','form.static',array('name'=>'filter','title'=>'Filter','text'=>$filterName)))
					->addUI('filtervalue', HALOUIBuilder::getInstance('','form.hidden',array('name'=>'filtervalue','title'=>'Filter','value'=>json_encode($filterValue))))
					->addUI('name', HALOUIBuilder::getInstance('','form.hidden',array('name'=>'name','title'=>'Name','placeholder'=>'Filter Name','value'=>$filter->name)))
					->addUI('type', HALOUIBuilder::getInstance('','form.select',array('name'=>'type','title'=>'Type','placeholder'=>'Filter Type','value'=>$filter->type, 'options' => array(array('title' => 'core', 'value' => 'core')))))
					...
					->addUI('on_display_handler', HALOUIBuilder::getInstance('','form.hidden',array('name'=>'on_display_handler','title'=>'Display Handler','placeholder'=>'Display Handler','value'=>$filter->on_display_handler)))
					->addUI('on_apply_handler', HALOUIBuilder::getInstance('','form.hidden',array('name'=>'on_apply_handler','title'=>'Apply Handler','placeholder'=>'Apply Handler','value'=>$filter->on_apply_handler)))
					...
					// ->addUI('params', HALOUIBuilder::getInstance('','form.text',array('name'=>'params','title'=>'Params','placeholder'=>'Params String','value'=>HALOParams::getInstance($filter->params)->toQuery(','))))
					->addUI('params', HALOUIBuilder::getInstance('','form.params',array('name'=>'params','title'=>'Params','value'=>HALOParams::getInstance($filter->params),'config'=>$filterParamsConfig)))
					;
	...
	*/
	public function ajaxUpdateFilterParams($filterValue){
		$filterValue = json_decode(html_entity_decode($filterValue), true);
		$filterConfig = HALOFilter::getFilterConfiguration();
		$filterParamsConfig = array_filter($filterConfig->filters, function($config) use($filterValue){
			if(isset($config['value'])){
				$value = json_decode($config['value'], true);
		...
		$filterParamsConfig = isset($filterParamsConfig['params'])?$filterParamsConfig['params']:array();
		//update params
		$builder = HALOUIBuilder::getInstance('','form.params',array('name'=>'params','title'=>'Params',
																	'value'=>HALOParams::getInstance(''),
																	'config'=>$filterParamsConfig));
