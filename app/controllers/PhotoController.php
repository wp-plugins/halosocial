...
class PhotoController extends BaseController
{
    ...
    /**
     * ajax handler to show edit album form
     *
     * @param  int $albumId
     * @return JSON
     */
    public function ajaxEditAlbumForm($albumId)
    {
        ...
        //detect edit/create mode
        $isCreate = empty($albumId) ? true : false;
        if ($isCreate) {
            $album = new HALOAlbumModel();
            $album->id = 0;//dummy album id
        } else {
            $album = HALOAlbumModel::find($albumId);
        }
        //permission checking
        ...
        $builder = HALOUIBuilder::getInstance('', 'form.form', array('name' => 'popupForm'))
            ->addUI('id', HALOUIBuilder::getInstance('', 'form.hidden', array('name' => 'id', 'value' => $album->id
            )))
            ->addUI('name', HALOUIBuilder::getInstance('', 'form.text', array('name' => 'name', 'id' => 'name', 'value' => $album->name,
                'title' => __halotext('Album Name'), 'validation' => 'required',
                'data' => array('confirm' => $album->name)
            )));

        $content = $builder->fetch();
        $actionSave = HALOPopupHelper::getAction(array("name" => __halotext('Save'), "onclick" => "halo.photo.saveAlbum()", "icon" => "check", 'class' => 'halo-btn-primary'));
        $title = $isCreate ? __halotext('Create New Album') : __halotext('Edit Album');
    ...
    public function ajaxSaveAlbum($postData)
    {
        $user = HALOUserModel::getUser();
        ...
        //detect edit/create mode
        $isCreate = empty($postData['id']) ? true : false;
        if ($isCreate) {
            $album = new HALOAlbumModel();
            $album->id = 0;//dummy album id
        } else {
            $album = HALOAlbumModel::find($postData['id']);
        }

        if (!HALOAlbumAPI::saveOrCreate($postData)) {
            return HALOResponse::sendResponse();
        }
        //get album model from HALOResponse
        $album = HALOResponse::getData('album');
        $actionSave = HALOPopupHelper::getAction(array("name" => __halotext('Go to the album page'), "onclick" => "halo.util.redirect('" . URL::to('?view=photo&task=album&uid=' . $album->id) . "')", "icon" => "check", 'class' => 'halo-btn-primary'));
        $title = $isCreate ? __halotext('Create New Album') : __halotext('Edit Album');
        $message = $isCreate ?  __halotext('Your album was created successfully') : __halotext('Your album was updated successfully');
        HALOResponse::addScriptCall('halo.popup.reset')
            ->addScriptCall('halo.popup.setFormTitle', $title)
            ->addScriptCall('halo.popup.setFormContent', $message)
            ->addScriptCall('halo.popup.addFormAction', $actionSave)
            ->addScriptCall('halo.popup.addFormActionCancel')
            ->addScriptCall('halo.popup.showForm');
    ...
    public function ajaxDeleteAlbum($postData)
    {
        $my = HALOUserModel::getUser();
        if (!$my) {
            HALOResponse::addMessage(HALOError::failed(__halotext('Login required.')));
        ...
        $albumId = Input::get('albumid', 0);
        $album = HALOAlbumModel::find($albumId);
        if (!$album) {
            HALOResponse::addMessage(HALOError::failed(__halotext('Unknown Album')));
            return HALOResponse::sendResponse();
        }

        //check permission: only album owner can delete album
        if (!$album->canEdit()) {
            HALOResponse::addMessage(HALOError::failed(__halotext("Only the album's owner can delete the album")));
            return HALOResponse::sendResponse();
        }

        $title = __halotext('Delete Album');
        ...
        if (!isset($postData['confirm'])) {
            ...
            HALOResponse::addScriptCall('halo.popup.reset')
                ->addScriptCall('halo.popup.setFormTitle', $title)
                ->addScriptCall('halo.popup.setMessage', __halotext('Do you really want to delete this album? All photos will be permanently deleted.'), 'error')
                ->addScriptCall('halo.popup.setFormContent', $content)
                ->addScriptCall('halo.popup.addFormAction', '{"name": "' . __halotext('Yes') . '","onclick": "halo.photo.deleteAlbum(\'' . $albumId . '\')","href": "javascript:void(0);"}')
                ->addScriptCall('halo.popup.addFormActionCancel')
                ->addScriptCall('halo.popup.showForm');

            return HALOResponse::sendResponse();
        } else {
            // Process oauth user have not password yet
            if (!empty($my->oauth) && !$my->getParams('has_password')) {
                    if (Input::get('force_delete') == 1) {
                        if ($album->delete()) {
                            $message = __halotext('Your album was permanently deleted');
                        } else {
                            $message = __halotext('An error occur while deleting your album');
                        }
                        HALOResponse::addScriptCall('halo.popup.reset')
                            ->addScriptCall('halo.popup.setFormTitle', $title)
                        ...
                        $builder = HALOUIBuilder::getInstance('editProfile', 'form.form', array('name' => 'popupForm'))
                        ->addUI('confirm', HALOUIBuilder::getInstance('', 'form.hidden', array('name' => 'confirm', 'value' => '')))
                        ->addUI('force_delete', HALOUIBuilder::getInstance('', 'form.hidden', array('name' => 'force_delete', 'value' => '1')));
                        $content = $builder->fetch();             
                        HALOResponse::addScriptCall('halo.popup.reset')
                        ->addScriptCall('halo.popup.setFormTitle', $title)
                        ->addScriptCall('halo.popup.setMessage', __halotext('You have not set a password yet. For security reasons we encourage you to set a password for your account!'), 'info')
                        ->addScriptCall('halo.popup.setFormContent', $content)
                        ->addScriptCall('halo.popup.addFormAction', '{"name": "' . __halotext('Set Password') . '","onclick": "halo.user.showChangePassword()","href": "javascript:void(0);"}')
                        ->addScriptCall('halo.popup.addFormAction', '{"name": "' . __halotext('Continue') . '","onclick": "halo.photo.deleteAlbum(\'' . $albumId . '\')","href": "javascript:void(0);"}')
                        ->addScriptCall('halo.popup.addFormActionCancel')
                        ->addScriptCall('halo.popup.showForm');

                        return HALOResponse::sendResponse();
                    }                    
            } else { // Process normal user and oauth user have password
                if ($my->verifyPassword(Input::get('confirm'))) {
                    if ($album->delete()) {
                        $message = __halotext('Your album was permanently deleted');
                    } else {
                        $message = __halotext('An error occur while deleting your album');
                    ...
                    if (empty($confirm)) {
                        $message = __halotext('Enter password to confirm deletion');
                        $messageError = null;
                    } else {
                        $message = __halotext('Enter password to confirm deletion');
                        $messageError = __halotext('Password incorrect'); 
                    }

                    $builder = HALOUIBuilder::getInstance('editProfile', 'form.form', array('name' => 'popupForm'))
                        ->addUI('confirm', HALOUIBuilder::getInstance('', 'form.password', array('name' => 'confirm', 'title' => $message, 'placeholder' => '', 'value' => '')))
                        ->addUI('link', HALOUIBuilder::getInstance('', 'form.link', array('url' => URL::to('/user/forgot'), 'title' => __halotext('Did you forget your password?'))));
                    $content = $builder->fetch();
                    $popup = HALOResponse::addScriptCall('halo.popup.reset')
                        ->addScriptCall('halo.popup.setFormTitle', $title);
                    if (!is_null($messageError)) {
                        $popup->addScriptCall('halo.popup.setMessage', $messageError, 'error', true);
                    }
                        $popup->addScriptCall('halo.popup.setFormContent', $content)
                                ->addScriptCall('halo.popup.addFormAction', '{"name": "' . __halotext('Delete') . '","onclick": "halo.photo.deleteAlbum(\'' . $albumId . '\')","href": "javascript:void(0);"}')
                                ->addScriptCall('halo.popup.addFormActionCancel')
                                ->addScriptCall('halo.popup.showForm');
    ...
    public function ajaxShowAlbumPhotos($albumId, $userId = null)
    {
        ...
        //load all album photos
        $album->load('photos');
        $photos = HALOPagination::getData($album->photos()->orderBy('created_at', 'desc'));
