...
class PhotoController extends BaseController
{
    ...
    public function ajaxShowEditAvatarForm($context, $target_id)
    {
        ...
        if ($mode == 'create') {
            ...
            $photo = $avatar->first();
            //redirect if photo doesn not exits
            Redirect::ajaxError(__halotext('Photo does not exists'))
                ->when(empty($photo->id))
                ->apply();
        }
        ...
        $actionUpload = HALOPopupHelper::getAction(array("name" => __halotext('Upload Photo'), "class" => "halo-btn-default halo-photo-upload-btn", "icon" => "cloud-upload"));
