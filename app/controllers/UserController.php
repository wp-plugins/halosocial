...
class UserController extends BaseController {
    ...
    {
        $redLogin = HALOConfig::get('user.redirectLogin', null);
        if (is_null($redLogin) && HALOResponse::isAjax()) {
            return '';
        }
    ...
    public function postLogin()
    {
        ...
        else
        {
            ...
            return Redirect::to('?view=user&task=login&code='. $err_code);
        }
    }
    ...
    public function postRegister()
    {
        ...
        //verify recaptcha
        if (!is_null(HALOReCaptcha::getSiteKey()) && !HALOReCaptcha::isVerified(Input::all())) {
            HALOResponse::addMessage(HALOError::failed(__halotext('Please enter recaptcha code to continue.')));
            return View::make('site/user/register', compact('input'));
        }
	...
	public function ajaxValidateNewUser($email, $username) {
		...
		$model = new UserModel();
		$validateRules = $model->getValidateRule();
		$rules = array('email' => $validateRules['email'], 'username' => $validateRules['username']);
	...
	*/
	...
	public static function getFacebookLogin() {
		...
		if ( !empty( $code ) ) {
			...
			$result = json_decode( $fb->request( '/v2.4/me?fields=email,name' ), true );

			//@todo: on invalid $result format checking
			
			$data = array();
			$data['uid'] = $result['id'];
			$data['consumer_id'] = HALO_OAUTH_CONSUMER_FB;
