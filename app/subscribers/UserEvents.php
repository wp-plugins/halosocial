			...
			if($checkAvatar) {
				if(!$my->hasAvatar()) {
					$editAvatarUrl = trim($my->getUrl());
						if($checkAvatar == 2 && ($currUrl != $editAvatarUrl)) {
							echo "<script> location.href = '" . $editAvatarUrl . "';</script>";
						}
					$editAvatarUrlHtml = '<a href="javascript:void()" class="halo-btn halo-btn-xs halo-btn-info" onclick="halo.photo.changeAvatar(\'user\',\'' . $my->user_id . '\')">' . __halotext('Change Your Avatar') . '</a>';
					echo HALOUIBuilder::getInstance('', 'alert', array('message' => sprintf(__halotext('You have not updated your avatar yet. Please consider to %s'), $editAvatarUrlHtml),
																		'type' => 'success'))->fetch();
				}
			...
			//check for required fields
				if(!$my->isProfileCompleted()) {
					$editProfileUrl = trim($my->getEditUrl());
						if($checkProfile == 2 && ($currUrl != $editProfileUrl)) {
							echo "<script> location.href = '" . $editProfileUrl . "';</script>";
						}
					$editProfileUrlHtml = '<a href="' . $editProfileUrl .'" class="halo-btn halo-btn-xs halo-btn-info">' . __halotext('Update Your Profile') . '</a>';
					echo HALOUIBuilder::getInstance('', 'alert', array('message' => sprintf(__halotext('You have not completed your profile yet. Please consider to %s'), $editProfileUrlHtml),
																		'type' => 'success'))->fetch();
				}
			}			
		}
