...
class BanEventHandler
{
    ...
    public function onCheckRole($role, $assetInstance, $user_id)
    {
        ...
        foreach ($settings as $setting) {
            if (isset($setting['roles']) && in_array($role->name, $setting['roles'])) {
                return false;
            }
        }
    }
    ...
    public function onLoadUserResponseActions($user)
    {
        if (HALOBanModel::canBan($user)) {
            echo HALOUIBuilder::getInstance('', 'user.responseActionItem', array('title' => __halotext('Ban'), 'icon' => 'minus-circle',
    ...
    public function onNotificationLoading(HALOObject $settings)
    {
        ...
        $settings->setNsValue('ban.submit.d', __halotext('Ban user from an action'));
    ...
    public function renderBanSubmit($notification)
    {
        ...
        if (empty($actors)) {
            ...
            $attachment->headline = sprintf(__halotext('%s banned you from "%s" for %s'), $actorsHtml, $target->getDescription(), '<span class="halo-countdown" data-countdown="' . $target->getDuration() . '"><span>');
    ...
    public function onLoadConfigurationSettings($settings)
    {
        ...
        $section = HALOUIBuilder::getInstance('bansettings', 'config.section', array('title' => __halotext('Settings')))
            ...
            'title' => __halotext('Enable ban feature'),
        ...
        $section = HALOUIBuilder::getInstance('banlevel1', 'config.section', array('title' => __halotext('Ban level 1')))
            ...
            'class' => 'selectpicker',
            'title' => __halotext('Ban roles'),
            'options' => $roles,
            'value' => HALOConfig::get('ban.level1.roles', ''))));
        $banCfg->builder->addUI('section@array', $section);

                            //ban level 2
        $section = HALOUIBuilder::getInstance('banlevel2', 'config.section', array('title' => __halotext('Ban level 2')))
                                ->addUI('ban.level2_desc', HALOUIBuilder::getInstance('', 'form.text', array('name' => 'ban.level2.description',
            'title' => __halotext('Description'),
            'value' => HALOConfig::get('ban.level2.description', 'Ban level 2'))))
        ...
        $banCfg->builder->addUI('section@array', $section);
                                        ...
                                        //ban level 3
        $section = HALOUIBuilder::getInstance('banlevel3', 'config.section', array('title' => __halotext('Ban level 3')))
                                            ->addUI('ban.level3_desc', HALOUIBuilder::getInstance('', 'form.text', array('name' => 'ban.level3.description',
            'title' => __halotext('Description'),
            'value' => HALOConfig::get('ban.level3.description', 'Ban level 3'))))
        ...
        $banCfg->builder->addUI('section@array', $section);
                                                    ...
                                                    //ban level 4
        $section = HALOUIBuilder::getInstance('banlevel4', 'config.section', array('title' => __halotext('Ban level 4')))
                                                        ->addUI('ban.level4_desc', HALOUIBuilder::getInstance('', 'form.text', array('name' => 'ban.level4.description',
            'title' => __halotext('Description'),
            'value' => HALOConfig::get('ban.level4.description', 'Ban level 4'))))
        ...
        $banCfg->builder->addUI('section@array', $section);
                                                                ...
                                                                //ban level 5
        $section = HALOUIBuilder::getInstance('banlevel5', 'config.section', array('title' => __halotext('Ban level 5')))
                                                                    ->addUI('ban.level5_desc', HALOUIBuilder::getInstance('', 'form.text', array('name' => 'ban.level5.description',
            'title' => __halotext('Description'),
            'value' => HALOConfig::get('ban.level5.description', 'Ban level 5'))))
