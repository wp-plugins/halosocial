...
class ConfigSeeder extends Seeder {
	...
	public function run()
	{
		...
		HALOConfig::set('global.activityDisplayLimit',20);
	...
	public function labelSeeder() {
		$allowedUsers = HALOUserModel::getModIds();
		...
		if(!$labelModel->params) {		//new create label
			$labelModel->params = json_encode(array('style' => 'danger', 'allowedRoles'=> $allowedUsers));
			$labelModel->save();
		}
		...
		$labelModel = HALOLabelModel::firstOrCreate($label);
		if(!$labelModel->params) {		//new create label
			$labelModel->params = json_encode(array('style' => 'success', 'allowedRoles'=> $allowedUsers));
			$labelModel->save();
		}
		...
		$labelModel = HALOLabelModel::firstOrCreate($label);
		if(!$labelModel->params) {		//new create label
			$labelModel->params = json_encode(array('style' => 'primary', 'lifetime'=> 12));
			$labelModel->save();
		}
