...
class HALOLabelAPI
{
    ...
    public static function assignLabels($target, $labelIds, $manualOnly = true)
    {
        ...
        $syncArray = array();
        $targetLabelAbles = array();
        $labelAbles = HALOLabelableModel::where('labelable_id', $target->id)->where('labelable_type', get_class($target))->get();
        foreach ($labelAbles as $able) {
            $targetLabelAbles[$able->label_id] = $able;
        }
        if (!empty($allowedLabelIds)) {
            foreach ($labelIds as $labelId) {
                if (in_array($labelId, $allowedLabelIds)) {
                    $created = Carbon::now();
                    if (isset($targetLabelAbles[$labelId])) {
                        $created = $targetLabelAbles[$labelId]->created_at;
                    }
                    $syncArray[$labelId] = array('params' => '', 'created_at' => $created, 'updated_at' => Carbon::now());
                }
            }
        }
