...
class HALOOutputHelper
{
	...
	public static function ellipsis($string, $len = 80, $ellipsis = ' ...'){
		...
		$truncate = Cache::remember($key,60,function() use($string,$len,$ellipsis){
			return self::truncate($string,$len,array('ending'=>$ellipsis,'exact'=>false,'html'=>true));
		});
		return $truncate;
	}
    ...
    public static function parseUrl($text)
    {
        ...
        foreach ($textarr as $piece) {
			...
			~xS';// The regex is a non-anchored pattern and does not have a single fixed starting character.
                ...
                $ret = preg_replace_callback($url_clickable, 'self::makeUrlClickableCb', $ret);

                $ret = preg_replace_callback('#([\s>])((www|ftp)\.[\w\\x80-\\xff\#$%&~/.\-;:=,?@\[\]+]+)#is', 'self::makeWebFtpClickableCb', $ret);
                $ret = preg_replace_callback('#([\s>])([.0-9a-z_+-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,})#i', 'self::makeEmailClickableCb', $ret);
