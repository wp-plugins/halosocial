...
class HALOBrowseHelper 
{
	...
	public static function fetchUrl($url, $cacheImage = true) 
	{
		...
		$info = Cache::remember('fetchUrl' . $key, 1, function () use ($url, $cacheImage) {			
			$rtn = HALOUtilHelper::preCatchFBShare($url);
			//check for oembed url
			$oembedHtml = wp_oembed_get($url, array());
			if($oembedHtml) {
				$info = new stdClass();
				$info->oembed = $oembedHtml;
				$info->url = $url;
				return $info;
			}
			...
			if ($my && $cacheImage) {
				...
				$images = array();
				if(method_exists($info, 'getImages')) {
					$listImages = $info->getImages();
					if(count($listImages)) {
						$images = (array)$listImages[0]['value'];
					}
				}
