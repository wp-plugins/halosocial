...
class HALOUtilHelper
{
	...
	public static function parseShareLink($postData, &$target) 
	{
		...
		if (!isset($postData['urlpreview']) && isset($target->message)) {
			...
			if (!empty($urls)) {
				...
				if ($info) {
					...
					$infoData['title'] = isset($info->title)?$info->title:'';
					$infoData['description'] = isset($info->description)?$info->description:'';
					if (isset($info->photos) && is_array($info->photos) && count($info->photos) > 0) {
					...
					if(isset($info->oembed) && $info->oembed) {
						$infoData['oembed'] = 1;
					}
		...
		if (isset($postData['urlpreview']) && isset($postData['urlpreview']['url'])) {
			...
			$data['oembed'] = isset($urlData['oembed']) ? $urlData['oembed'] : 0;
	...
	*/
    ...
    /**
     * Get activated menu item on navbar by checking existed menu in definition
     *
     * @param  string $nav
     * @return string
     */
    public static function getActiveNav($nav = 'default')
    {
        $view = Input::get('view', '');
        $task = Input::get('task', '');
        $path = $view . '/' . $task;
        ...
        if (isset($regex[$nav]) && preg_match($regex[$nav], $path)) {
            return 'active';
        }

        foreach ($regex as $key => $value) {
            if (preg_match($regex[$nav], $path)) {
                return 'active';
            }
        }
        return '';
	...
	/*
		return client ip address for this current request
	*/
