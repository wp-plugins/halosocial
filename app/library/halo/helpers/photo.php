...
class HALOPhotoHelper 
{
			...
			if (!file_exists($nDir)) {
				...
				if ($width == 'center') {
					...
					if ($currWidth < $currHeight) {
						...
						$newWidth = $height;
						$newHeight = ceil($currHeight * $height / $currWidth);
						$top = ceil(($newHeight - $height) / 2);
						$left = 0;
					} else {
						//use currHeight as based size
						$newHeight = $height;
						$newWidth = ceil($currWidth * $height / $currHeight);
						$top = 0;
						$left = ceil(($newWidth - $height) / 2);
					}
					//resize the image
	...
	{
		$image = "assets/images/site-banner.jpg";
