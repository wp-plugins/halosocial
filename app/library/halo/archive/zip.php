...
class HALOArchiveZip extends HALOObject
{
	/**
	...
	function extract($archive, $destination, $options = array ())
	{
		...
		if ($this->hasNativeSupport()) {
			return ($this->_extractNative($archive, $destination, $options))? true : $this->get('error.message');
		} else {
			return ($this->_extract($archive, $destination, $options))? true : $this->get('error.message');
		}
	}
				...
				while ($file = @zip_read($zip))
				{
					if (zip_entry_open($zip, $file, "r")) {
						if (substr(zip_entry_name($file), strlen(zip_entry_name($file)) - 1) != "/") {
							...
							if (File::writeFile($destination.'/'.zip_entry_name($file), $buffer) === false) {
								$this->set('error.message', 'Unable to write entry');
								return false;
							}
