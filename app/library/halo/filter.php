...
class HALOFilter
{
	...
	*/
	...
	/*
		function to get filter configuration from events
	*/
	public static function getFilterConfiguration(){
		$filterConfig = new stdClass();
		$filterConfig->filters = array();
		Event::fire('filter.config.load', array(&$filterConfig));

		//convert filters array to json string
		$filterConfig->filters = array_map(function($config){
		...
		$filterConfig->filters = array_filter($filterConfig->filters, function($config){
			return isset($config['title']) && isset($config['value']);
		});
		return $filterConfig;
	}
}
