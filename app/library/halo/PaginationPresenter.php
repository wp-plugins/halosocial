...
class HALOPaginationPresenter
{
    ...
    /**
     * Get the sortable link url
     *
     * @param  string   Link text
     * @param  string   the sort string used in model
     * @return void
     */
    public static function getSortableLink($paginator, $title, $sortString, $dir = 'asc')
    {

        $currSort = Input::get('sort', '');
        $currDir = Input::get('dir', '');
        $dirIcon = '';
        if ($sortString == $currSort) {
            $dirIcon = '<i class="fa fa-sort-amount-' . $currDir . '"></i>';
            $dir = ($currDir == 'asc') ? 'desc' : 'asc';
        }

        $paginator = clone $paginator;
        $url = $paginator->appends(array('sort' => $sortString, 'dir' => $dir))->getUrl(1);

        return '<a href="' . $url . '">' . $title . ' ' . $dirIcon . '</a>';
    }
