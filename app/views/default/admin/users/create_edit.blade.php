...
<div class="panel panel-default">
	...
	<form name="halo-admin-form" id="halo-admin-form" method="post" action="@if (isset($user)){{ URL::to('?app=admin&view=users&task=edit&uid=' . $user->id) }}@endif">
		<!-- CSRF Token -->
		...
		$generalTabContent = HALOUIBuilder::getInstance('','content',array())
							->addUI('username',HALOUIBuilder::getInstance('','form.text',array('name'=>'username','id'=>'username','value'=> Input::old('username', isset($user) ? $user->getUserName() : null),
							...
							->addUI('email',HALOUIBuilder::getInstance('','form.text',array('name'=>'email','id'=>'email','value'=> Input::old('email', isset($user) ? $user->getEmail() : null),
							...
							->addUI('profile_id',HALOUIBuilder::getInstance('','form.select',array('name'=>'profile_id','id'=>'profile_id','value'=> Input::old('profile_id', (isset($user) && !empty($user->profile_id)) ? $user->profile_id : HALOProfileModel::getDefaultProfileId('user')),
																	'title'=>__halotext('User Profile Type'),
																	'options'=>HALOProfileModel::getProfileListOption('user',true)
																	)))
							->addUI('point_count',HALOUIBuilder::getInstance('','form.text',array('name'=>'point_count','id'=>'point_count','value'=> Input::old('point_count', isset($user) ? $user->point_count : 0),
																	'title'=>__halotext('User Point'),'placeholder'=>__halotext('User Point'), 'class'=>($errors->has('point_count') ? 'error' : ''))))
							;
		$generalTabContent->addUI('block',HALOUIBuilder::getInstance('','form.radio',array('name'=>'block','id'=>'block','value'=> $user->block,
																	'title'=>'Block User','class'=>($errors->has('block') ? 'error' : ''),
