        ...
        for (y=0;y<btnGroups.length;y++) {
          ...
          for (z=0;z<buttons.length;z++) {
            var button = buttons[z],
                ...
                btnClass = button.btnClass ? button.btnClass : 'halo-btn',
            ...
            btnGroupContainer.append('<button type="button" class="'
                                    +btnClass
                                    +' halo-btn-default halo-btn-sm" title="'
  ...
  , showEditor: function() {
      ...
      if (this.$editor == null) {
        ...
        var editorHeader = $('<div/>', {
                            'class': 'md-header halo-btn-toolbar' + ((typeof this.$options.headerclass == 'undefined')?'': ' ' +this.$options.headerclass)
                            })
        ...
        if (options.savable) {
          ...
          editorFooter.append('<button class="halo-btn halo-btn-success" data-provider="'
                              +ns
                              +'" data-handler="'
                              +saveHandler
        ...
        this.$editor.on('click', '[data-provider="'+this.$ns+'"]', $.proxy(this.__handle, this))
    ...
    buttons: [
      ...
      , {
          ...
          data: [{
            ...
            title: "Emoticons",
            icon: { glyph: 'glyphicon glyphicon-search', fa: 'fa fa-smile-o', 'fa-3': 'icon-smile-o' },
            callback: function(e){
              // Replace selection with some drinks
							var $editorWrapper = e.$element.closest('.md-editor');
							var buttonHandler = e.$ns+'-'+ 'cmdEmoticons';
							var $ePopover = jQuery('[data-handler="'+buttonHandler+'"]',$editorWrapper).hpopover(
							{
								template: '<div class="popover emoticons-popover"><div class="arrow"></div><div class="popover-content"></div></div>'
								,content: function(){
									var html = '';
									return html;
								}
							...
							.on('shown.bs.hpopover',function(){
								...
								var $emojiContainer = $this.next();
								var $wrapper = jQuery('.popover-content',$emojiContainer)
								
								var cursor, selected = e.getSelection()
								if(!$wrapper.children().length){
									for(var i = 0; i < halo.emoji.popular.length; i++){
										$wrapper.append(jQuery('<a href="javascript:void(0)" >' + emojify.replace(':' + halo.emoji.popular[i] + ':') + '</a>')
														.on('click',function(ele){
															var $this = jQuery(this);
															var emojiName = jQuery('img',$this).attr('title');
															e.replaceSelection(emojiName);
															$ePopover.hpopover('toggle');
														})
										);
									}
								}
