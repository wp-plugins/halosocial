...
jQuery.extend(true, halo, {
	user: {
		...
		, postRegister: function(ele){
			jQuery(ele).append(halo.template.getLoadingIcon());
			halo.form.submit('registerForm');
		}
	}
});
				...
				jQuery(this).haloMagnificPopup({
					...
					iframe: {
                        markup: '<div class="mfp-figure">' +
                            '<div class="halo-gallery-popup-content halo-video-popup-content" style="margin:0px;">' +
                            '   <div class="mfp-close"></div>' +
                            '   <div class="halo-gallery-body">' +
                            '       <div class="halo-gallery-left">' +
                            '           <div class="mfp-iframe-scaler"><iframe class="mfp-iframe" frameborder="0" allowfullscreen></iframe><div class="clearfix"></div></div>' +
                            '           <div class="mfp-bottom-bar clearfix"></div>'+
                            '       </div>' +
						...
						change: function () {
                            ...
                            var $wrapper = jQuery('.halo-gallery-right', '.halo-video-popup-content').find('div.halo-gallery-right-wrapper');
                            $wrapper.attr('data-halozone', 'popup_comment.' + context + '.' + videoId);
                            ...
                            var loadCb = function() {
                                ...
                                var $wrapper = jQuery('.halo-gallery-right .halo-gallery-right-wrapper', '.halo-video-popup-content');
                                var $iframeWrap = $wrapper.closest('.halo-video-popup-content').find('.mfp-iframe-scaler');
                                var $rightContainer = $wrapper.closest('.halo-gallery-right-container');
                                var containerOHeight = parseInt($wrapper.css('height'));
                                var newHeight = parseInt($iframeWrap.css('height')) - parseInt($rightContainer.css('paddingTop')) * 2;

                                // if (parseInt($wrapper.css('height')) > parseInt($iframeWrap.css('height'))) {
                                    jQuery('.halo-comment-input', $wrapper).css({position: 'absolute', left: parseInt($rightContainer.css('paddingLeft')) + 'px', bottom: 0, width: $wrapper.css('width')});
                                    $rightContainer.css('position', 'relative');
                                    $wrapper.css('padding-bottom', (parseInt(jQuery('.halo-comment-input', $wrapper).css('height')) - parseInt($rightContainer.css('paddingBottom')) + parseInt(jQuery('.halo-comment-input', $wrapper).css('paddingBottom'))) + 'px');
                                // }
                                $wrapper.css({
                                    overflowY: 'auto',
                                    overflowX: 'hidden',
                            ...
                            var resetCb = function() {
                                var $wrapper = jQuery('.halo-gallery-right .halo-gallery-right-wrapper', '.halo-video-popup-content');
                                $wrapper.css({overflowY: '', overflowX: '', height: '', paddingBottom: ''});
                                jQuery('.halo-comment-input', $wrapper).css({width: '', position: ''});
                            };

                            var scrollToLastCommentCb = function() {
                                jQuery('.halo-gallery-right .halo-gallery-right-wrapper', '.halo-video-popup-content').scrollTop(99999);
                            };

							halo.jax.call('video', 'LoadPopupContent', videoId, halo.jax.successCb(function() {
                                loadCb();
                                scrollToLastCommentCb();
                                jQuery(window).resize(function(evt) {
                                    loadCb();
                                });
                                jQuery(document).bind('updateLayout.halo.comment', function() {
                                    loadCb();
                                    scrollToLastCommentCb();
                                });
                            }));
...
jQuery.extend(true, halo, {
	photo: {
		...
		editAlbumForm: function(albumId) {
		    if (typeof albumId == 'undefined') albumId = 0;
            halo.popup.setConfirmClosing(function() {
                var confirmation = halo.util.validateDataConfirmation(".halo-popup-content");
                confirmation.message = __halotext('You have unsaved changes. Are you sure you want to continue?');
                if (!confirmation.success) {
                    confirmation.input.closest('.form-album').addClass('has-error');
                }
                return confirmation;
            });
            halo.jax.call("photo", "EditAlbumForm", albumId);
		},
        saveAlbum: function () {
            var values = halo.util.getFormValues('popupForm');
            halo.jax.call("photo", "SaveAlbum", values);
        },
        deleteAlbum: function (albumId) {
            var values = halo.util.getFormValues('popupForm');
            halo.util.setFormValue(values, 'albumid', albumId);
            halo.jax.call("photo", "DeleteAlbum", values);
        }, 
...
jQuery.extend(true, halo, {
	util: {
        ...
        uniqueContent: function(content, attr) {
            var rtn = [];
            var tmp = [];
            var i = 0;
            while (i < content.length) {
                var key = parseInt(jQuery(content[i]).attr(attr));
                if (jQuery.inArray(key, tmp) == -1) {
                    rtn.push(content[i]);
                    tmp.push(key);
                }
                i++;
		...
		getHtmlData: function(data, key) {
		    var dataAttr = [];
		    if (data && typeof data == 'object') {
		        for (var name in data) {
		            if (name == key || typeof key == 'undefined')
		                dataAttr.push('data-' + name + '="' + data[name] + '"');
		        }
		    }
...
jQuery.extend(true, halo, {
	jax: {
		...
		submitTask: function (comName, func, postData, responseFunc, cacheKey) {
			...
			var req = jQuery.ajax({
				...
				.always(function (data, textStatus) {
					...
					halo.jax.onCompletedLoading(comName, func);
				});
		...
		onCompletedLoading: function(comName, func) {
		    jQuery(document).trigger(comName + '.' + func + '.haloLoading');
		}
	}
...
jQuery.extend(true, halo, {
	popup: {
		...
		addFormAction: function (opts) {
			...
			data = halo.util.getHtmlData(options.data);
			var action = '<button ' + data + ' type="button" class="halo-action-btn halo-btn ' + options['class'] + '" onclick="' + options.onclick + '">' + icon + options.name + '</button>';
...
jQuery.extend(true, halo, {
	form: {
        ...
        subscribeNews: function() {
            var values = halo.util.getFormValues('popupForm');
            halo.jax.call("admin,dashboard", "SubscribeNewsLetter", values);
        },
        initFormValidation: function(scope) {
            jQuery('.haloj-form-validation', scope).validate({
                onkeyup: false
            });
        },
		...
		init: function (scope) {
            ...
            //validate form
            halo.form.initFormValidation(scope);
		}
	}
});
...
jQuery.extend(true, halo, {
	template: {
		...
		init: function (scope) {
			...
			halo.template.initLoading(scope);
		},
		...
		getJaxLoading: function (zoom) {
			return '<div class="halo-jax-loading"><div class="halo-loading-icon-wrapper">' + halo.template.getLoadingIcon(zoom) + '</div></div>';
		}, 
        ...
        initLoading: function(scope) {
             jQuery('[data-loading]', scope).each(function() {
                 ...
                 $that.on('click', function(evt) {
                     ...
                     if ($container.find('.halo-featured-loading').length) return false;
                     ...
                     var loadingWrap = '<span ' + attr + ' ></span>';
                     */

                     var loadingContent = '<span class="halo-featured-loading"><i class="halo-featured-loading-icon fa fa-spinner fa-spin ' + sizeClass + '"></i>' + title + '</span>';
                     $that.off('show.halo.haloLoading').on('show.halo.haloLoading', function(evt) {
                        if (replace) {
                            $container.empty();
                        }
                     ...
                     if (autoShow) {
                        $that.trigger('show.halo.haloLoading');
                     }
                     if (complete) {
                         jQuery(document).bind(complete, function(evt) {
                            jQuery('.halo-featured-loading', $container).remove();
                            if (replace) {
...
jQuery.extend(true, halo, {
	zone: {
		initZone: function (zone) {
            if (typeof zone == 'object' && zone.data('halozone')) {
                var zoneId = zone.data('halozone');
                jQuery('[data-halozone="' + zoneId + '"]').each(function() {
                    halo.init(jQuery(this));
                });
            } else {
                halo.init(zone);
            }
