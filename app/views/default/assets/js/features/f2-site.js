	...
	comment: {
		...
		submit: function (el) {
			...
			halo.jax.call("comment", "Submit", values, halo.jax.successCb(function(){
                ...
                jQuery(document).trigger('updateLayout.halo.comment');
			}));
		},
		viewall: function (id, context) {
			halo.jax.call("comment", "ViewAll", id, context, halo.jax.successCb(function(){
                jQuery(document).trigger('updateLayout.halo.comment');
            }));
		},
		'delete': function (id) {
...
jQuery.extend(true, halo, {
	share: {
		...
		, addShareLinkBlock: function(ele,url) {
            ...
            $input.trigger('show.halo.haloLoading');
...
jQuery.extend(true, halo, {
	status: {
		...
		init: function (scope) {
			...
			jQuery('.halo-status-box', scope).one('focus', function (e) {
