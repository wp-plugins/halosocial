...
jQuery.extend(true, halo, {
	filter: {
		...
		updateFilterParams: function(ele){
			var value = jQuery(ele).val();
			var valueObj = jQuery.parseJSON( value );
			if(typeof valueObj['display'] !== 'undefined' && typeof valueObj['apply'] !== 'undefined' && typeof valueObj['name'] !== 'undefined'){
				var form = jQuery(ele).closest('#popupForm');
				form.find('[name="on_display_handler"]').val(valueObj.display);
				form.find('[name="on_apply_handler"]').val(valueObj.apply);
				form.find('[name="name"]').val(valueObj.name);
			}
			halo.jax.call("admin,filters", "UpdateFilterParams", value);
		},
