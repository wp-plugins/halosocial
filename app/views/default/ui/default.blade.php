...
<div class="panel-body haloj-content-block">
	<div class="halo-activity">
		...
		<div class="halo-stream-content">
			<!-- message content -->
			<div class="halo-stream-message" data-more-link="<a href='javascript:void(0);'>{{__halotext('Read More')}} </a>">
...
<span class="halo-like-btn" title="{{{$builder->title}}}" {{$builder->getOnClick()}}>{{$builder->getIcon()}} {{HALOUIBuilder::getInstance('',$builder->like.'count',array('liked'=>$builder->liked,'likeList'=>$builder->likeList,'numberOnly'=>$builder->numberOnly))->fetch()}}</span>
</span>
@endUI
			...
			<div class="input-group clearfix halo-btn-block">
				<div class="halo-pull-right @if($visiableFilterHtml ==='') hidden @endif">
					<button type="button" class="halo-btn halo-btn-xs halo-btn-nobg halo-dropdown-toggle" data-htoggle="filter"
					        data-filter-content="halo_filter_content{{$uid}}" title="{{__halotext('Filters')}}">
...
{{-- ///////////////////////////////////// Ajax login UI ///////////////////////////////////// --}}
@beginUI('ajaxlogin')
<form class="" name="{{$builder->name}}" id="{{$builder->name}}" accept-charset="UTF-8"
	...
	<fieldset>
		...
		{{HALOUIBuilder::getInstance('remember_me', 'form.checkbox', array('name' => 'remember', 'value' => 1,
												'options' => array(HALOObject::getInstance(array('value'=>'1','title'=>__halotext('Remember Me'))))
		))->fetch()}}
		...
		<div class="form-group">
			<button tabindex="3" type="submit" class="halo-btn halo-btn-primary halo-btn-block">{{ __halotext('Login') }}</button>
		</div>
		<div class="form-group">
		...
		<div class="form-group text-center">
			<hr/>
			<div class="text-center"> {{__halotext('Or Sign in with')}}</div>
			@foreach(HALOUtilHelper::getSettingList($socialSettings,'social') as $secName => $secValue)
...
<div class="halo-section-container halo-section-{{$builder->name}}">
	...
	@endif
	<div class="halo-section-action halo-section-list-toolbar clearfix">
		{{$builder->actions}}
	</div>
...
<div class="halo-share-toggle">
	...
	<ul class="halo-dropdown-menu" role="menu">
		...
		@endforeach
		{{-- Share by Email --}}
		<li class="halo-share-item-wrapper" data-name="email" data-counter="0" data-url="{{$builder->target->getUrl()}}">
			{{HALOUIBuilder::getInstance('', 'social.share_email', array('post' => $builder->target))->fetch()}}
		</li>
	</ul>
</div>
@endif
