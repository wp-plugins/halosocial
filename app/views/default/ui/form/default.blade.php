...
{{-- ///////////////////////////////////// Form Basic UI: ReCaptcha ///////////////////////////////////// --}}
@beginUI('form.recaptcha')
<?php $uid = HALOUtilHelper::uniqidInt(); ?>
<div class="form-group {{$builder->class}} {{$builder->getSize()}} {{($builder->getErrorClass())}}">
    <label for="" class="required">{{$builder->title ? $builder->title : __halotext('Captcha')}}</label>
    <div class="">   
        <div id="html_element"></div>
        <input {{$builder->getValidation()}} type="hidden" class="halo-recaptcha-response {{$builder->getValidationLabel()}}" name="{{ $builder->name }}" id="halo-recaptcha-{{$uid}}" value=""/>
        <script type="text/javascript">
            var reCaptchaVerifyCb = function(response) {
                jQuery('#{{"halo-recaptcha-$uid"}}').val(grecaptcha.getResponse()).trigger('change');
            };
            var onloadCallback = function() {
                grecaptcha.render('html_element', {
                    'sitekey' : '{{HALOReCaptcha::getSiteKey("6LcBRQwTAAAAAIsIo-x32Xil6hSW3Q696r1Ov4Me")}}',
                    'callback': reCaptchaVerifyCb,
                    'expired-callback': reCaptchaVerifyCb
                });
            };
        </script>
...
@beginUI('form.params')
<?php
	$paramConfig = is_null($builder->config)?array():$builder->config;

	$paramValue = json_decode(json_encode($builder->value), true);
	$paramValue = (isset($paramValue['params']) && is_array($paramValue['params']))?$paramValue['params']:array();
?>
<div class="form-params" data-halozone="form.params">
	@if(!empty($paramConfig))
		<h4 class="text-center"> Parameters</h4>
		@foreach($paramConfig as $title => $config)
			<?php 	$ui = isset($config['ui'])?$config['ui']:'form.text'; 
					$uiData = array();
					$uiData['name'] = 'params[' . $title . ']';
					$uiData['title'] = ucfirst($title);
					if(isset($paramValue[$title])){
						$uiData['value'] = $paramValue[$title];
					}
					$uiData = array_merge($config, $uiData);
					if(isset($uiData['ui'])){
						$ui = $uiData['ui'];
					}
			?>
			{{HALOUIBuilder::getInstance('', $ui, $uiData)->fetch()}}
		@endforeach
	@else
