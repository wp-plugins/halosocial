...
class HALOBanModel extends HALOModel
{
    ...
    public static function banActions($user_id, $data)
    {
        ...
        if (empty($data['reason'])) {
            $error = HALOError::getMessageBag()->add('reason', __halotext('Restrictions reason must be defined'));
        ...
        foreach ($settings as $action => $setting) {
            ...
            if ($index === false) {
                ...
                if (is_numeric($data['ban_duration'][$index]) && ($duration = intval($data['ban_duration'][$index])) > 0) {
                    ...
                    $error = HALOError::getMessageBag()->add('ban_duration[' . $index . ']', __halotext('Please enter a valid  number'));
                    HALOResponse::addMessage($error);
                    return false;
                }
            }
    ...
    public static function getSettings($user = null)
    {
        ...
        if ($user && !empty($__settings[$user->id])) {
            return $__settings[$user->id];
        }
    ...
    public function getDuration()
    {
        ...
        $seconds = Carbon::now()->diffInSeconds($this->expired_at, false);
        if ($seconds > 0) {
            return $seconds;
        }
        return 0;
