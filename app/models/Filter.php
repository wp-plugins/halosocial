...
class HALOFilterModel extends HALOModel
{
    ...
    public function bindData($postData = array())
    {
        ...
        if (isset($postData['params'])) {
			if(is_array($postData['params'])){
				$this->params = new HALOParams($postData['params'], 'array');
			} else {
	...
	*/
	public function getDisplayName(){
		...
		$filterValue = array('display' => $this->on_display_handler, 'apply' => $this->on_apply_handler, 'name' => $this->name);
		...
		if(!empty($filterParamsConfig) && isset($filterParamsConfig['title'])){
			return $filterParamsConfig['title'];
		}
		return $this->name;
	}
}
