...
class HALOUserModel extends HALOModel
{
    ...
    /**
     * HALOOnlineuserModel, HALOUserModel: one to many
     * 
     * @return Illuminate\Database\Eloquent\Relations\hasMany
     */
    public function onlineUsers()
    {
        return $this->hasMany('HALOOnlineuserModel', 'user_id');
    }
    ...
    {
        $default = array('username' => 'required', 'email' => 'required');
        if (HALOConfig::get('user.displayName.minlen', 0)) {
            $displayName[] = 'min:' . HALOConfig::get('user.displayName.minlen');
        }
        if (HALOConfig::get('user.displayName.maxlen', 0)) {
            $displayName[] = 'max:' . HALOConfig::get('user.displayName.maxlen');
        }
        if (HALOConfig::get('user.displayName.required', 0)) {
            $displayName[] = 'required';
        }
        if (isset($displayName)) {
            $default = array_merge($default, array('display_name' => implode('|', $displayName)));
        }
        return $default;
    }
    ...
    public function getAvatar($size = HALO_PHOTO_AVATAR_SIZE)
    {
        ...
        if ($user->avatar_id && $user->avatar) {
            ...
            $rtn = HALOPhotoHelper::getCropPhotoURL($path, $size, $size, $user->getParams('avatarZoom', 100), $user->getParams('avatarTop', 0), $user->getParams('avatarLeft', 0), $user->getParams('avatarWidth', 0));
			if($rtn){
				return $rtn;
			}
        } else {
    ...
    public function getCover($size = HALO_PHOTO_COVER_SIZE, $ratio = 0.375)
    {
        ...
        if ($user->cover_id && $user->cover) {
            ...
            $rtn = HALOPhotoHelper::getCropPhotoURL($path, $size, round($size * $ratio), $user->getParams('coverZoom', 100), $user->getParams('coverTop', 0), $user->getParams('coverLeft', 0), $this->getParams('coverWidth', 0));
			if($rtn) {
				return $rtn;
			}
        }
