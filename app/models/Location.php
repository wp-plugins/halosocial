...
class HALOLocationModel extends HALOModel
{
    ...
    public function getDistrict($reload = true)
    {
		...
		$that = $this;
        return Cache::rememberForever('location.district.' . $this->id, function () use($that) {
            if (Schema::hasTable('vnm_adm3')) {
                //postgis enabled
                $address = $that->getAddress();
                $condition = "ST_Contains(polygon, ST_SetSRID(ST_MakePoint('" . $that->lng . "','" . $that->lat . "'),4326))";
                $district_name = DB::table('vnm_adm3')->whereRaw($condition)->pluck('name_3');
                if ($district_name) {
                    $that->district_id = 9999;
                    $that->district_name = $district_name;
                    $that->save();
                    return $that->district_name;
                ...
                if ($district) {
                    ...
                    $that->district_name = $district->name;
                    $that->save();
    ...
    public function getCity($reload = true)
    {
		...
		$that = $this;
        return Cache::rememberForever('location.city.' . $this->id, function () use($that) {
            if (Schema::hasTable('vnm_adm3')) {
                //postgis enabled
                $address = $that->getAddress();
                $condition = "ST_Contains(polygon, ST_SetSRID(ST_MakePoint('" . $that->lng . "','" . $that->lat . "'),4326))";
