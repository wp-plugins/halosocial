...
if(!has_halo_pkgs()) {
	...
	define ('HALO_PLUGIN_VER','1.0.6');

	define ('HALO_RELEASE_DATE', '2015-10-06');
	...
	function halo_theme_comp_head() {
		HALOAssetHelper::printHtmlHeader();
	}
    ...
    // Delete user
    function halo_delete_user($user_id) {
        haloBoot();
    ...
    add_action('delete_user', 'halo_delete_user');
	...
	function halo_dismiss_notice(){
        if (isset($_POST['notice']) && isset($_POST['type'])) {
            $key = 'halosocial_notice_' . md5($_POST['notice'] . $_POST['type']);
            update_option($key, 1);
            exit;    
        }
...
//nav bar menu
/* if(file_exists(dirname(__FILE__) . '/halo_nav.php')){
	require_once(dirname(__FILE__) . '/halo_nav.php');
}
 */
/****************************** Checking for plugin recover stored folders and files  ***************************/
if (file_exists(dirname(__FILE__) . '/halo_update.php') && is_admin()) {
    require_once(dirname(__FILE__) . '/halo_update.php');
}

// Load translations
function halo_load_textdomain() {
    load_plugin_textdomain( 'halosocial', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' ); 
}
add_action( 'plugins_loaded', 'halo_load_textdomain' );

// Redirect all the registration links to Halo registration
add_filter('register_url', function($register_url) {
    $registerFrm = get_option('halo_register_form', 0);
    if ($registerFrm && ($url = get_option('halo_register_url'))) {
        return $url;
    }
    return $register_url;
}, 50);
...
function halo_ping_enqueue_scripts() {
	?>
...
add_action( 'wp_head', 'halo_ping_enqueue_scripts' );


add_action('init', 'halo_ping_init');
function halo_ping_init(){
	if(is_user_logged_in()){
		// add_action('wp_footer', 'halosocial_enqueue_scripts');
		wp_enqueue_script( 'halo_ping', plugins_url( 'app/views/default/assets/js/ping.js', __FILE__ ), array( 'jquery' ), false, true );
	}	
}
if(is_admin()){
	add_action('wp_ajax_halo_ping', 'halo_ping_process');
	add_action('wp_ajax_nopriv_halo_ping', 'halo_ping_process');
}
function halo_ping_process(){
	if(is_user_logged_in()){
		if ( isset($_REQUEST) ) {
			require_once(dirname(__FILE__) . '/app/library/halo/helpers/utilities.php');
			global $wpdb;
			...
			$status = 0;
			$whereClause = array('user_id' => $user_id, 'session' => $session );
			$whereFormat = array('%d', '%s');
			$onlineTable = $wpdb->prefix . 'halo_online_users';
			$exists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $onlineTable WHERE user_id = %d AND session = %s", $user_id, $session ) );
			if($exists){
				$updateData = array('updated_at' => $now);
				$updateFormat = array('%s');
				$wpdb->update( $onlineTable, $updateData, $whereClause, $updateFormat, $whereFormat );
			} else {
				$ip_addr = HALOUtilHelper::getClientIpAddress();
				$insertData = array('user_id' => $user_id, 'ip_addr' => $ip_addr, 'client_type' => $client_type, 
									'status' => $status, 'session' => $session, 'b_userAgent' => $browser['b_userAgent'],
									'b_name' => $browser['b_name'], 'b_version' => $browser['b_version'], 'b_platform' => $browser['b_platform'],
									'params' => '', 'created_at' => $now, 'updated_at' => $now);
				$insertFormat = array(	'%d', '%s', '%d', 
										'%d', '%s', '%s', 
										'%s', '%s', '%s',
										'%s', '%s', '%s');
				$wpdb->insert( $onlineTable, $insertData, $insertFormat );
			}
			if($userids){
				echo halo_getOnlineJson($userids);
			}
		}
	}
	exit;
...
function halo_flushOnlineList(){
	global $wpdb;
	$onlineTable = $wpdb->prefix . 'halo_online_users';
	$now = current_time( 'mysql' );
	$intervalMin = 5;
	$isClear = wp_cache_get('halo_check_online', 'halo');
	if($isClear === false){
		$wpdb->get_results($wpdb->prepare("DELETE FROM $onlineTable WHERE updated_at + INTERVAL $intervalMin MINUTE <= %s", $now));
		wp_cache_set('halo_check_online', 1, 'halo', 60*3*$intervalMin);
	}
}

function halo_getOnlineJson($userids){
	...
	if(!empty($userids)){
		...
		$onlineList = array();
		$offlineList = explode(',', $userids);
		$rows = $wpdb->get_results($wpdb->prepare("SELECT * FROM $onlineTable WHERE updated_at + INTERVAL $intervalMin MINUTE > %s
												AND user_id in (%s)", $now, $userids));
		$onlineUsers = array_map(function($row){ 
									return $row->user_id;
								}, $rows);
