...
if(!has_halo_pkgs()) {
	...
	define ('HALO_PLUGIN_VER','1.0.0_RC2_starter');

	define ('HALO_RELEASE_DATE', '2015-07-13');
	...
	function halo_admin_notice() {
		...
		if(HALO_PLUGIN_PRODUCT_TYPE === 'STARTER'){
			$message = sprintf("You are using HaloSocial Starter. To get access to all the great features like events, groups, pages, messages, classifieds and more, please upgrade to Professional or Agency versions. [%s]", $comparisonLink);
		} else {
		...
		if($message){
		?>
		<div class="updated halo-starter-notice halo-notice-hide">
	...
	function halo_user_login_message( $message ) {
		...
		if ( isset( $_GET['blocked'] ) && $_GET['blocked'] == 1 ) {
			$message =  '<div id="login_error">' . __( 'Your account has been blocked. Please contact administrator for more information.') . '</div>';
		}
...
if(get_option( 'edd_halo_social_license_status')) {
	// setup the updater
	$edd_updater = new EDD_SL_Plugin_Updater( EDD_HALOSOCIAL_STORE_URL, __FILE__, array( 
